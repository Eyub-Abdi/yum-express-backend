...
const updateAdminPhone = async (req, res) => {
  ...
  // 4. Generate OTP and expiry
  const { code, expiry } = generateOtp(5) // 5-digit OTP, 10-minute expiry

  // 5. Update the admin record with the new phone and OTP
  await knex('admins').where({ id }).update({
    phone,
    otp_code: code,
    otp_expiry: expiry,
  ...
  // 6. Send OTP via SMS
  const otpMessage = `Yum Express: Your verification code is ${code}`
  try {
    await sendSMS(phone.replace('+', ''), otpMessage)
  } catch (err) {
