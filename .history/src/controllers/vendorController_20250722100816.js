...
const getVendorHours = async (req, res) => {
  ...
  if (todayHours && todayHours.open_time && todayHours.close_time) {
    ...
    // Sync DB if open state differs from DB value
    if (is_open && todayHours.is_closed) {
      await knex('vendor_hours').where({ vendor_id, category: todayCategory }).update({ is_closed: false, updated_at: new Date() })
    } else if (!is_open && !todayHours.is_closed) {
      await knex('vendor_hours').where({ vendor_id, category: todayCategory }).update({ is_closed: true, updated_at: new Date() })
    }
  }
