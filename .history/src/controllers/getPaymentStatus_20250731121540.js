...
const getPaymentStatus = async (req, res) => {
  ...
  if (!orderReference && !transaction_id) {
    return res.status(400).json({ message: 'Missing orderReference or transaction_id' })
  }

  const payments = await knex('payments')
    .where(builder => {
      if (orderReference) {
        builder.where('order_reference', orderReference).orWhereIn('order_id', function () {
          this.select('id').from('orders').where('order_reference', orderReference)
        })
      }
      if (transaction_id) {
        builder.orWhere('transaction_id', transaction_id)
      }
    })
    .orderBy('created_at', 'desc')
    .limit(1)
