...
const getAllRiderOrders = async (req, res) => {
  const riderId = req.params.id || 1
  const { status, date } = req.query

  try {
    let query = knex('deliveries as d').select('d.id as delivery_id', 'd.order_id', 'd.status as delivery_status', 'd.address', 'd.phone', 'd.created_at as delivery_created_at', 'd.delivered_at', 'd.estimated_time', 'd.delivery_notes', 'd.lat', 'd.lng', 'o.total_price', 'o.order_status', 'o.payment_status', 'o.created_at as order_created_at', knex.raw("CONCAT(c.first_name, ' ', c.last_name) as customer_name"), knex.raw("CONCAT(v.first_name, ' ', v.last_name) as vendor_name"), 'v.id as vendor_id', 'c.id as customer_id', knex.raw('EXTRACT(EPOCH FROM (d.delivered_at - d.created_at))/60 as delivery_time_minutes')).leftJoin('orders as o', 'd.order_id', 'o.id').leftJoin('customers as c', 'd.customer_id', 'c.id').leftJoin('vendors as v', 'd.vendor_id', 'v.id').where('d.assigned_to', riderId).orderBy('d.created_at', 'desc')

    if (status) query.andWhere('d.status', status)
    if (date) query.andWhereRaw('DATE(d.created_at) = ?', [date])
...
// Note: The above code assumes that the rider ID is passed as a URL parameter (e
