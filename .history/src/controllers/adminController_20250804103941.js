...
const { adminRegistrationSchema, updateAdminSchema, nameSchema } = require('../schemas/adminSchema')
...
const updateAdminEmail = async (req, res) => {
  ...
  const emailTaken = await knex('admins').where({ new_email: email }).andWhereNot({ id }).first()
  if (emailTaken) {
