...
const registerAdmin = async (req, res) => {
  ...
  const { first_name, last_name, email, phone, role } = value
  const password = generateDefaultPassword()
  console.log(`Generated default password for admin: ${password}`)

  // Check if admin already exists
  const existing = await knex('admins').where({ email }).orWhere({ phone }).first()
  if (existing) {
  ...
  // Build and send SMS
  const message = buildWelcomeMessage(first_name, 'admin', password)
  console.log(message)
  ...
  try {
    await sendEmail({
      ...
      type: 'verification',
      payload: { token: verification_token, entityType: 'admins' }
    })
  } catch (err) {
...
const toggleAdminStatus = async (req, res) => {
  ...
  await knex('admins').where({ id }).update({ is_active: newStatus, updated_at: knex.fn.now() })

  res.json({ message: `Admin status updated`, is_active: newStatus })
}
...
module.exports = { registerAdmin, getAllAdmins, getAdminById, getAdminProfile, updateAdmin, deleteAdmin, updateAdminPassword, verifyAdminEmail, updateAdminName, updateAdminEmail, updateAdminPhone, toggleAdminStatus }
