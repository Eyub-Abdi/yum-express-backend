...
const { updateVendorHourSchema, vendorUpdateSchema, vendorEmailUpdateSchema, vendorLocationSchema } = require('../schemas/vendorSchema')
...
const updateVendorHours = async (req, res, next) => {
  const { error } = hoursSchema.validate(req.body)
  if (error) return res.status(400).json({ error: error.details[0].message })
