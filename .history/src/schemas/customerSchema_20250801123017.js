...
const customerRegistrationSchema = Joi.object({
  ...
  phone: Joi.string()
    .pattern(/^[1-9]\d{5,14}$/) // starts with 1-9, total 6-15 digits
    .required()
    .messages({
      'string.base': 'Phone number must be a string.',
      'string.empty': 'Phone number is required.',
      'string.pattern.base': 'Phone number must start with the country code (no + or 0) and be 6 to 15 digits long.'
    }),
